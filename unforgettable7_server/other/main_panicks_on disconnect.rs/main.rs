...
use unforgettable7_common::{WsMessage};
...
async fn main() {
    ...
    info!(
        "unforgettable7 http server listening on {} and WebSocket on /unforgettable7ws/",
    ...
    // WebSocket server
    // GET from route /unforgettable7ws/ -> WebSocket upgrade
    let websocket = warp::path("unforgettable7ws")
        // The `ws2()` filter will prepare WebSocket handshake...
        .and(warp::ws())
        .and(users)
        // Match `/unforgettable7ws/url_param` it can be any string.
    ...
    let fileserver = warp::fs::dir("./unforgettable7/");
...
fn user_message(msg_sender_ws_uid: usize, message: Message, users: &Users) {
    ...
    // There are different messages coming from the unforgettable7 wasm app
    ...
    match msg {
        ...
        WsMessageGameData::MsgPong { .. } => {
            unreachable!("unforgettable7_server must not receive MsgPong");
        }
        WsMessageGameData::MsgResponseWsUid { .. } => {
